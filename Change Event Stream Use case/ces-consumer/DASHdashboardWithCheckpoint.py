...
def update_static(n):
    ...
    if not readings_df.empty:
        high = readings_df[readings_df["TemperatureCelsius"] > 28.0]
        alerts = dbc.Alert(
            f"Wykryto {len(high)} pomiarów powyżej 28°C!" if not high.empty else "Brak krytycznych temperatur.",
    ...
    if selected_sensor and not readings_df.empty:
        ...
        if s:
            desc = html.Div([
                ...
                html.P(f"Lokalizacja: {s['Location']}"),
                html.P(f"Model: {s['Model']}"),
                html.P(f"Zainstalowano: {s['InstallDate']}"),
                html.P(f"Aktywny: {'TAK' if s['IsActive'] else 'NIE'}"),
            ])
